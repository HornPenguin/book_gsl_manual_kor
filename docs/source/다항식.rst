...
.. c:function:: double gsl_poly_eval (const double c[], const int len, const double x)

  실수 값 :code:`x` 대해, 실수 계수를 가지는 다항식의 값을 계산합니다. 

.. c:function:: gsl_complex gsl_poly_complex_eval (const double c[], const int len, const gsl_complex z)

  허수 값 :code:`z` 대해, 실수 계수를 가지는 다항식의 값을 계산합니다.

.. c:function:: gsl_complex gsl_complex_poly_complex_eval (const gsl_complex c[], const int len, const gsl_complex z)

  허수 값 :code:`z` 대해, 허수 계수를 가지는 다항식의 값을 계산합니다.

.. c:function:: int gsl_poly_eval_derivs (const double c[], const size_t lenc, const double x, double res[], const size_t lenres)
...
.. c:function:: int gsl_poly_dd_init (double dd[], const double xa[], const double ya[], size_t size)
...
.. c:function:: double gsl_poly_dd_eval (const double dd[], const double xa[], const size_t size, const double x)
...
.. c:function:: int gsl_poly_dd_taylor (double c[], double xp, const double dd[], const double xa[], size_t size, double w[])
...
.. c:function:: int gsl_poly_dd_hermite_init (double dd[], double za[], const double xa[], const double ya[], const double dya[], const size_t size)
...
.. c:function:: int gsl_poly_solve_quadratic (double a, double b, double c, double * x0, double * x1)
...
.. c:function:: int gsl_poly_complex_solve_quadratic (double a, double b, double c, gsl_complex * z0, gsl_complex * z1)
...
.. c:function:: int gsl_poly_solve_cubic (double a, double b, double c, double * x0, double * x1, double * x2)
...
.. c:function:: int gsl_poly_complex_solve_cubic (double a, double b, double c, gsl_complex * z0, gsl_complex * z1, gsl_complex * z2)
...
.. c:function:: gsl_poly_complex_workspace * gsl_poly_complex_workspace_alloc (size_t n)
...
.. c:function:: void gsl_poly_complex_workspace_free (gsl_poly_complex_workspace * w)

  작업 공간 :code:`w` rk 할당된 모든 메모리를 해제합니다.


.. c:function:: int gsl_poly_complex_solve (const double * a, size_t n, gsl_poly_complex_workspace * w, gsl_complex_packed_ptr z)
