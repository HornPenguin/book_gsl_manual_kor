...
.. c:function:: gsl_permutation * gsl_permutation_alloc (size_t n)
...
.. c:function:: gsl_permutation * gsl_permutation_calloc (size_t n)
...
.. c:function:: void gsl_permutation_init (gsl_permutation * p)

   주어진 순열 :data:`p` 를 항등 순열로 초기화합니다. 
   :code:`n` 크기의 순열에 대해, :math:`(0, 1, 2, \dots, n - 1)` 로 초기화 합니다.


.. c:function:: void gsl_permutation_free (gsl_permutation * p)

   순열 :data:`p` 의 메모리를 해제합니다.

.. c:function:: int gsl_permutation_memcpy (gsl_permutation * dest, const gsl_permutation * src)
...
.. c:function:: size_t gsl_permutation_get (const gsl_permutation * p, const size_t i)
...
.. c:function:: int gsl_permutation_swap (gsl_permutation * p, const size_t i, const size_t j)
...
순열의 성질
==============================
.. c:function:: size_t gsl_permutation_size (const gsl_permutation * p)

   주어진 순열 :data:`p` 의 크기를 반환합니다.
   

.. c:function:: size_t * gsl_permutation_data (const gsl_permutation * p)
...
.. c:function:: int gsl_permutation_valid (const gsl_permutation * p)
...
.. c:function:: void gsl_permutation_reverse (gsl_permutation * p)
...
.. c:function:: int gsl_permutation_inverse (gsl_permutation * inv, const gsl_permutation * p)

   주어진 순열 :data:`p` 의 역순열을 계산해 :data:`inv` 에 저장합니다.

.. index:: iterating through permutations

.. c:function:: int gsl_permutation_next (gsl_permutation * p)

   This function advances the permutation :data:`p` to the next permutation
   in lexicographic order and returns :c:marco:`GSL_SUCCESS`.  If no further
   permutations are available it returns :c:marco:`GSL_FAILURE` and leaves
   :data:`p` unmodified.  Starting with the identity permutation and
   repeatedly applying this function will iterate through all possible
   permutations of a given order.

.. c:function:: int gsl_permutation_prev (gsl_permutation * p)

   This function steps backwards from the permutation :data:`p` to the
   previous permutation in lexicographic order, returning
   :c:marco:`GSL_SUCCESS`.  If no previous permutation is available it returns
   :c:marco:`GSL_FAILURE` and leaves :data:`p` unmodified.
...
.. c:function:: int gsl_permute (const size_t * p, double * data, size_t stride, size_t n)

   순열 :data:`p` 를 길이 :data:`n` , :data:`stride` 크기의 걸음을 가지는 값 배열 :data:`data` 에
   적용합니다.


.. c:function:: int gsl_permute_inverse (const size_t * p, double * data, size_t stride, size_t n)

   순열 :data:`p` 의 역순열을 길이 :data:`n` , :data:`stride` 크기의 걸음을 가지는 
   값 배열 :data:`data` 에 적용합니다.

.. c:function:: int gsl_permute_vector (const gsl_permutation * p, gsl_vector * v)
...
.. c:function:: int gsl_permute_vector_inverse (const gsl_permutation * p, gsl_vector * v)
...
.. c:function:: int gsl_permute_matrix (const gsl_permutation * p, gsl_matrix * A)
...
.. c:function:: int gsl_permutation_mul (gsl_permutation * p, const gsl_permutation * pa, const gsl_permutation * pb)
...
.. c:function:: int gsl_permutation_fwrite (FILE * stream, const gsl_permutation * p)
   ...
   :c:marco:`GSL_EFAILED` if there was a problem writing to the file.  Since the
   data is written in the native binary format it may not be portable
   between different architectures.

.. c:function:: int gsl_permutation_fread (FILE * stream, gsl_permutation * p)
   ...
   :c:marco:`GSL_EFAILED` if there was a problem reading from the file.  The
   data is assumed to have been written in the native binary format on the
   same architecture.

.. c:function:: int gsl_permutation_fprintf (FILE * stream, const gsl_permutation * p, const char * format)
   ...
   The function returns :c:marco:`GSL_EFAILED` if there was a problem writing
   to the file.

.. c:function:: int gsl_permutation_fscanf (FILE * stream, gsl_permutation * p)
   ...
   :c:marco:`GSL_EFAILED` if there was a problem reading from the file.
...
.. c:function:: int gsl_permutation_linear_to_canonical (gsl_permutation * q, const gsl_permutation * p)

   This function computes the canonical form of the permutation :data:`p` and
   stores it in the output argument :data:`q`.

.. c:function:: int gsl_permutation_canonical_to_linear (gsl_permutation * p, const gsl_permutation * q)
...
.. c:function:: size_t gsl_permutation_inversions (const gsl_permutation * p)
...
.. c:function:: size_t gsl_permutation_linear_cycles (const gsl_permutation * p)

   This function counts the number of cycles in the permutation :data:`p`, given in linear form.

.. c:function:: size_t gsl_permutation_canonical_cycles (const gsl_permutation * q)
