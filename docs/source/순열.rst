...
.. c:function:: int gsl_permutation_next (gsl_permutation * p)
   ...
   in lexicographic order and returns :c:macro:`GSL_SUCCESS`.  If no further
   permutations are available it returns :c:macro:`GSL_FAILURE` and leaves
...
.. c:function:: int gsl_permutation_prev (gsl_permutation * p)
   ...
   :c:macro:`GSL_SUCCESS`.  If no previous permutation is available it returns
   :c:macro:`GSL_FAILURE` and leaves :data:`p` unmodified.
...
.. c:function:: int gsl_permutation_fprintf (FILE * stream, const gsl_permutation * p, const char * format)
   ...
   The function returns :c:macro:`GSL_EFAILED` if there was a problem writing
   to the file.
