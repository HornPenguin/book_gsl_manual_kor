...
이 단원에서 기술하는 함수들은 헤더 파일 :math:`gsl_deriv.h` 에 기술되어 있습니다.
...
공간에서 :math:`x-h, x-\frac{h}{2}, x, x+ \frac{h}{2}, x+h` 지점을 의미합니다. 
각각의 미분 과정에서 오차의 계산은 5개 지점 방법과 3개 지점 :math:`x-h, x, x+h` 방법의 차이를 이용해 계산합니다. 
...
공간에서 :math:`x+\frac{h}{4}, x+\frac{h}{2}, x+\frac{3h}{4}, x+h` 지점을 의미합니다. 
각각의 미분 과정에서 오차의 계산은 4개 지점 방법과 2개 지점 :math:`x+\frac{3h}{4}, x+h` 방법의 차이를 이용해 계산합니다. 
