...
수학 참고 문헌으로 본 라이브러리에서 사용하는 문헌은 Abramwotiz & Stegun 의 저서를 표준으로 사용하고 있습니다.
해당 문헌은 좋은 문헌이지만 새로운 갱신이 중단된 서적입니다. 

이 서적에 대체제로 NIST에서 디지털 문헌으로 Digital Library of Mathematical Functions(DLMF)를 제공하고 있으며, 
Cambridge University Press에서 해당 문헌을 NIST Handbook of Mathematical Functions란 이름으로 출판된 서적을 제공합니다.

이외에 참고 가능한 디지털 문헌으로 Wolfram Math World가 있습니다. CRC


https://encyclopediaofmath.org/wiki/Main_Page 는 유럽 수학회에서 관리하는 온라인 수학 백과입니다.
...
**GMPAL** 는 GNU Multiple Precision Arithmetic Library의 약자입니다.  
정밀한 수학 계산을 할 때 많은 경우 double, float 형의 유효 숫자의 한계가 생깁니다. 
GMPAL는 이러한 고정밀 대수 연산을 위한을 라이브러리로 메모리의 제한을 무시한다면 정밀도의 한계 없이 계산을 할 수 있습니다. 
GMPAL 라이브러리의 주된 활용 분야는 암호, 인터넷 보안, 대수학과 계산 대수학 등이 있습니다
 
https://gmplib.org/


**Libmatheval** 는 심볼릭 연산을 위한 C, Fortran 라이브러리 입니다.

 https://www.gnu.org/software/libmatheval/

**Gnuplot** 은 오픈소스 그래픽 소프트웨어입니다. 
C api를 제공해 C에서 여러 그래프와 3D 플롯을 그리는 데 사용할 수 있습니다.

http://www.gnuplot.info/

**GTK+** 는 Gimp의 그래픽 인터페이스를 위해 시작된 그래픽 라이브러리로 본격적인 소프트웨어의 GUI를 만드는 데 유용하며, 
이를 이용해 사용자 정의 플롯 라이브러리를 짤 수도 있습니다.

https://www.gtk.org/

**HDF5** 는 대용량 데이터 처리를 위한 계층적 파일 형식으로 C api를 지원합니다. 
막대한 데이터를 다루고자 할때, 이러한 전문 파일 형식의 사용은 크게 유용합니다.

https://www.hdfgroup.org/solutions/hdf5/

**GLPK** 는 GNU Linear Programming Kit의 약자입니다. 
선형 프로그래밍을 위한 여러 기능들을 제공합니다. 

https://www.gnu.org/software/glpk/

**NLopt** 은 Non linear optimization을 위한 라이브러리입니다. 
여러가지 최적화 방법들을 적은 인터페이스로 사용할 수 있는 기능을 제공하며, 
C, C++, Fortran, Matlab, GNU Octave, Python, GNU Cuile, Juliaa, R, Lia, OCaml 그리고 Rust를 지원합니다. 


https://nlopt.readthedocs.io/en/latest/

**FFTW** 는 Fastest Fourier Transform in the West의 약자로 Fast Fourier Transform 기능을 구현한 라이브러리입니다.
...
**Mathematica** 는 기호 계산을 위한 프로그램 중 가장 광범위 하게 쓰이는 소프트웨어입니다.
...
근레, Wolfram 사는 개인 연구가와 프로그래머들을 위한 Wolfram engine을 무료로 공개했습니다.
