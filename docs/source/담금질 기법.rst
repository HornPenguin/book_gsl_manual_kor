...
뉴턴 방법과 같은 방법은 사용할 수 없습니다(야코비안 같은 미분 행렬 계산이 필요합니다.). 
이런 경우에 확률적 탐색 기법를 시도해 볼 수 있습니다. 
특히 이 방법들은, 조합적 최적화 문제를 푸는 데 빈번히 사용됩니다. 
이러한 문제의 예로 외판원 문제이 있습니다.
...
전이 과정에서 온도는 설계된 냉각 절차에 따라 천천히 낮아집니다. 
...
.. c:type:: gsl_siman_Efunc_t
   ...
   현재 상태 :code:`xp` 에 대한 에너지를 반환해야 합니다.
...
.. c:type:: gsl_siman_step_t
   ...
   현재 상태 :code:`xp` 를 난수 발생자 :code:`r` 와 
   단계별 최대 이동거리 :code:`step_size` 를 이용해 수정해야 합니다.
...
.. c:type:: gsl_siman_metric_t
   ...
   주어진 두 개의 상태 :code:`xp` 와 :code:`yp` 사이의 거리를 반환해야 합니다.
...
.. c:type:: gsl_siman_print_t
   ...
   주어진 상태 :code:`xp` 의 설정값을 출력해야 합니다.
...
.. c:type:: gsl_siman_copy_t
   ...
   설정 값을 :code:`source` 에서  :code:`dest` 로 복사해야 합니다.
...
이는 의도된 사항입니다. 이 구현체는 C로 작성해 C에서 호출함을 염두에 두고 개발되었으며, 
동시에 다양한 응용 가능성에 목적을 두고 있기 때문입니다. 
그렇기에 여기서 라이브러리의 사용자들이 개발하는 응용 프로그램에 약간의 수정을 거쳐 적용할 수 있는 
...
때문에, 지오이드 거리 [#Geoid]_ 를 적용하지 않았습니다.
...
.. [#Geoid] 해발고도를 재는 기준이 되는 지구의 구형 높이 입니다(*).