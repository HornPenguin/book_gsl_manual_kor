...
이 단원에서는 물리 상수들을 위한 매크로들을 기술합니다. 예를 들어 빛의 속도  :math:`c` 나, 중력 상수  :math:`G`  등이 있습니다. 각각의 값들은 다른 단위계들로 존재합니다. 천체물리에서 빈번히 쓰이는 표준 MKSA 계 (meter, kilogransms, seconds, ampers)와 CGSM 계 (centimeters, grams, seconds, gauss)가 있습니다.

 MKSA 계로 정의된 함수들은  ``gsl_const_mksa.h``  정의되어 있습니다. CGSM 계로 정의된 상수들은  ``gsl_const_cgsm.h``  정의되어 있습니다. 차원이 없는 단위, 예를 들어서 구조 상수와 같은 순수한 숫자들은  ``gsl_const_num.h``  정의되어 있습니다.
...
진공 투자율  :math:`\mu_0` . 이 상수는 MKSA 계에서만 정의되었습니다.

>**GSL_CONST_MKSA_VACUUM_PERMITTIVITY**

진공 유전율  :math:`\epsilon_0` . 이 상수는 MKSA 계에서만 정의되었습니다.
...
에너지 단위 리드버그 수,  :math:`R_y` . 리드버그 상수와 다음과 같은 관계를 가진다.  :math:`R_y =hcR_{\infty}`   
...
전자의 자기 모멘트 절대 값,  :math:`\mu_e` . 실제 물리적인 전자의 자기 모멘트는 음수 값을 가진다.
