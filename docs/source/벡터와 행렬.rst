...
.. function:: int gsl_block_fwrite (FILE * stream, const gsl_block * b)
  ...
  블럭 :code:`b` 원소 값들을 스트림 :code:`stream` 바이너리 형태로 작성합니다. 
...
.. function:: int gsl_block_fread (FILE * stream, gsl_block * b)
  ...
  길이로 할당되어 있어야합니다. 왜냐하면, 이 함수는 :code:`b` 길이를 
...
이 함수는 길이  :math:`n` 의 벡터를 만들고, 벡터 구조체의 시작 지점을 가르키는 포인터를 반환합니다. 벡터의 원소로 새로운 블럭이 할당되고 벡터 구조체의 :code:`block` 변수에 저장됩니다. 이때 블럭은 벡터에 "소유된" 상태입니다. 만약, 벡터가 해제된다면 이 블럭도 같이 해제됩니다. 크기가  :math:`0` 인 요청도 수행되며 :code:`non-null` 결과를 반환합니다.
...
이 함수는 할당된 벡터 :code:`v` 해제합니다. 만약 벡터가 :code:`gsl_vector_alloc()`  통해 만들어졌다면 벡터 안의 블럭도 같이 해제됩니다. 만약, 벡터가 메모리의 다른 객체로부터 만들어졌고 다른 객체가 소유권을 가지고 있다면 블럭은 해제되지 않습니다.

벡터 원소에 접근
--------------------------

포트란 컴파일러들과는 다르게 C 컴파일러들은 벡터와 행렬들에 대해 범위 확인 기능을 지원하지 않습니다^[range]. 함수 :code:`gsl_vector_get()`  :code:`gsl_vector_set()`  간단한 범위 확인 기능을 제공하고, 범위 밖 요소에 접근할 경우 오류를 보고합니다.

벡터와 행렬의 원소에 접근하는 함수들은 :code:`gsl_vector.h`  정의 되어 있으며, :code:`extern inline`  인라인 선언 되어 있습니다. 이러한 구조는 함수 호출 과정에서 오버헤드를 줄여줍니다. 이를 위해서는 프로그램을 컴파일 할 때, 전처리기에서 매크로 :code:`HAVE_INLINE` 처리 하도록 선언해야합니다.


>GSL_RANGE_CEHCK_OFF


만약 필요한 경우 :code:`GSL_RANGE_CHECK_OFF` 정의해 재컴파일하면 코드의 수정 없이 모든 범위 확인 기능을 무시할 수 있습니다. 인라인 함수를 지원하는 컴파일러에서 이러한 범위 확인 기능을 끄는 것은 :code:`gsl_vector_get(v,i)`  :code:`gsl_vector_set(v,i,x)`  각각 :code:`v->data[i*v->stride]`  :code:`v->data[i*v->stride]=x`  바꾸는 것과 같습니다. 따라서, 범위 확인 기능을 해제한 상황에서 범위 확인 기능이 있는 함수를 사용해도 성능상의 하락은 없습니다.


>GSL_C99_INLINE


C99 컴파일러를 사용한다면 :code:`extern inline` 대신 :code:`inline` 키워드를 사용해 인라인 함수를 정의합니다. 이 경우 매크로 :code:`GSL_C99_INLINE` 정의해야합니다. GCC의 C99 모드 :code:`-std=c99` 에서는 자동으로 선택됩니다. 


>int **gsl_check_range

만약 인라인 함수들을 사용하지 않으면, :code:`gsl_vector_get()`  :code:`gsl_vector_set()` 함수는 라이브러리에 존재하는 컴파일된 버전으로 연결됩니다. 이러한 경우, 범위 확인 기능은 전역 정수 변수 :code:`gsl_check_range` 의해 제어됩니다. 기본적으로 이 옵션은 활성화 되어있으며 :code:`gsl_check_range`  :math:`0` 으로 설정해 기능을 끌 수 있습니다. 함수 호출 과정에서 오버헤드의 문제 때문에, 인라인 함수와 비교해서 범위 확인 기능을 끄는 것은 권장되지 않습니다.


>double **gsl_vector_get**(const gsl_vector * *v*, const size_t *i*)


이 함수는 벡터 :code:`v` :code:`i` 번째 원소를 반환합니다. 만약, :code:`i`  :math:`0` 에서 :code:`size-1` 밖에 있다면, 오류 관리자가 실행되고  :math:`0` 이 반환됩니다. :code:`HAVE_INLINE` 정의된 경우에는 인라인 함수가 사용됩니다. 
...
이 함수는 벡터 :code:`v` :code:`i` 번째 원소를 :code:`x` 설정합니다. 만약, :code:`i`  :math:`0` 에서 :code:`size-1` 밖에 있다면, 오류 관리자가 실행됩니다. :code:`HAVE_INLINE` 정의된 경우에는 인라인 함수가 사용됩니다. 
...
이 함수들은 벡터 :code:`v` :code:`i` 번째 원소를 가르키는 포인터를 반환합니다.  만약, :code:`i`  :math:`0` 에서 :code:`size-1` 밖에 있다면, 오류 관리자가 실행되고 :code:`NULL` 포인터가 반환됩니다.  :code:`HAVE_INLINE` 정의된 경우에는 인라인 함수가 사용됩니다. 
...
이 함수는 벡터 :code:`v` 모든 원소를 주어진 값 :code:`x` 초기화합니다.

>void **gsl_vector_set_zero**(gsl_vector * *v*)

이 함수는 벡터 :code:`v` 모든 원소를  :math:`0` 으로 초기화합니다.

>int **gsl_vector_set_basis**(gsl_vector * *v*. size_t *i*)

이 함수는 벡터 :code:`v` 원소중 :code:`i` 째 원소를  :math:`1` 로, 나머지 원소를  :math:`0` 으로 초기화해, 기저 벡터를 만듭니다.
...
이 함수는 벡터 :code:`v` 원소들을 스트림 :code:`stream` 이진 형식의 내용으로 작성합니다. 작성에 오류가 없으면  :math:`0`  값을 반환하고, 작성에서 오류가 발생했을 시 :code:`GSL_EFAILED` 값을 반환합니다. 해당 값들은 작동하는 시스템의 이진 형식을 따르기 때문에, 다른 시스템끼리의 호환성은 보장되지 않습니다.

>int **gsl_vector_fread**(FILE * *stream*, gsl_vector * *v*)

이 함수는 열린 스트림 :code:`stream` 값들을 읽어 주어진 벡터 :code:`v` 이진 형태의 값으로 저장합니다. 벡터 :code:`v` 사전에 할당되어 있어야하며, 읽을 값의 크기에 맞추어 적절한 길이를 가지고 있어야합니다. 왜냐하면, 이 함수가 값을 읽는 길이는 :code:`v` 길이에 의존하기 때문입니다. 작성에 오류가 없으면  :math:`0`  값을 반환하고, 작성에서 오류가 발생했을 시 :code:`GSL_EFAILED` 값을 반환합니다. 읽을 값은 같은 시스템에서 사용하는 이진 형식으로 작성되었다라 가정합니다.

>int **gsl_vector_fprintf**(FILE * *stream*, const gsl_vector * *v*, const char * *format*)

이 함수는 주어진 벡터 :code:`v` 원소들을 정해진 형식 :code:`format` 로 스트림 :code:`stream` 한 줄씩 기록합니다. :code:`format` 부동 소수점을 위한 :code:`%g`  :code:`%e`  :code:`%f`  있고, 정수형은 :code:`%d`  사용할 수 있습니다. 작성에 오류가 없으면  :math:`0`  값을 반환하고, 작성에서 오류가 발생했을 시 :code:`GSL_EFAILED` 값을 반환합니다.

>int **gsl_vector_fscanf**(FILE * *stream*, gsl_vector * *v*)

이 함수는 :code:`stream` 로부터 형식화된 값을 읽어 벡터 :code:`v` 저장합니다. 벡터 :code:`v` 사전에 할당되어 있어야하며, 읽을 값의 크기에 맞추어 적절한 길이를 가지고 있어야합니다. 왜냐하면, 이 함수가 값을 읽는 길이는 :code:`v` 길이에 의존하기 때문입니다. 작성에 오류가 없으면  :math:`0`  값을 반환하고, 작성에서 오류가 발생했을 시 :code:`GSL_EFAILED` 값을 반환합니다.
...
벡터 view는 stack에 저장되는 임시 개체로 벡터 원소들의 부분 집합에 적용할 수 있는 연산입니다. 이러한 벡터 view는 상수 벡터와 비상수 벡터 모두에 적용할 수 있습니다. 각각의 성질 보존을 위해 자료형을 분리해 제공합니다. 일반적인 벡터는 :code:`gsl_vector_view` 상수 벡터는 :code:`gsl_vector_const_view` 자료형을 사용합니다. 두 경우 모두, view의 원소는 view 객체의 :code:`vector` 분을 사용해 :code:`gsl_vector` 접근할 수 있습니다. :code:`gsl_vector *`  :code:`const gsl_vector *` 포인터는 이 성분들에 :code:`&` 산을 적용해 얻을 수 있습니다.

이 포인터를 사용할 때는, view가 스코프 안에 있는지 확인하는 것이 매우 중요합니다. 가장 간단한 방법은 포인터를 :code:`&view.vector` 태로 사용하고, 다른 변수에 해당 값을 저장하지 않는 것입니다.
...
이 함수들은 벡터 :code:`v` 부분 벡터에 대한 벡터 view를 반환합니다. 새로운 벡터의 시작 값은 원본 벡터의 시작 지점으로부터 거리 :code:`offset` 이용해 정해집니다. 새 벡터는 :code:`n` 의 원소를 가지고 있습니다. 수학적으로 새로운 벡터 :code:`v'`  :code:`i` 번째 원소는 다음과 같이 주어집니다.
...
.. code-block:: c
 ...
 :code:`i` :code:`0` 서 :code:`n-1` 지 입니다.

반환된 벡터 구조체의 :code:`data` 포인터는  :code:`offset` :code:`n`  계수가 원본 벡터의 끝을 넘을 경우 :code:`null` 초기화됩니다.

새로운 벡터는 원본 벡터 :code:`v` 이루는 블럭들의 view일 뿐입니다. :code:`b` 원소를 이루는 블럭들은 새로운 벡터가 소유하기 있지 않습니다. view가 스코프 밖으로 나간다 하더라도, 원본 벡터 :code:`v` 블럭들은 여전히 존재합니다. 원본 벡터가 담긴 메모리는 오직 원본 벡터의 해제로만 해제가 가능합니다. 물론, 원본 벡터도 만약 해당 벡터의 view가 사용되고 있을 때에는 해제가 불가능합니다.

함수 :code:`gsl_vector_const_subvector()`  :code:`gsl_vector_subvector()`  동일합니다. 하지만, :code:`const` 선언된 벡터에 대해서 사용가능합니다.
...
이 함수들은 보폭 값 :code:`stride` 이용해 다른 벡터 :code:`v` 서 생성된 부분 벡터의 벡터 view를 반환합니다. 이 부분 벡터는 :code:`gsl_vector_subvector()`  같은 방식으로 생성되고 :code:`n` 의 원소를 가지게 됩니다. 이는 원래 벡터의 한 원소에서 다음 원소까지 :code:`stride` 간격을 가지도록 추출된 원소들입니다. 수학적으로 새 벡터 :code:`v'`  :code:`i` 째 원소는 다음과 같이 주어집니다. 
...
.. code-block:: c
 ...
 :code:`i`  :math:`0` 부터 :code:`n-1` 지의 범위를 가집니다.

유의할 점은 부분 벡터의 view들은 원래 벡터의 원소들에대한 직접 접근을 제공한다는 점입니다.

예를 들어서, 다음의 예제 코드는 길이 :code:`n` 벡터 :code:`b` 짝수번째 원소들을  :math:`0` 으로 만듭니다. 홀수 번째 원소들은 건드리지 않습니다.
...
벡터 view들은 :code:`&view.vector`  사용해 벡터를 인자로 가지는 모든 함수들에 할당된 다른 벡터처럼 인자로 넣어줄 수 있습니다. 예를 들어 다음의 코드는 벡터 :code:`v` 홀수 번째 원소들에 대해 BLAS 함수 :code:`dnrm2` 사용해 노름을 계산합니다.
...
함수 :code:`gsl_vector_const_subvector_with_stride()`  :code:`gsl_vector_subvector_with_stride()`  동일한 기능을 제공합니다. 하지만 :code:`const` 정의된 벡터들에서만 사용가능합니다.
...
이 함수들은 복소수 벡터 :code:`v` 대해 실수부로 이루어진 벡터 view를 반환합니다.

함수 :code:`gsl_vector_complex_const_real()`  :code:`gsl_vector_complex_real()`  동일한 기능을 제공합니다. 하지만 :code:`const` 정의된 벡터들에서만 사용가능합니다. 
...
이 함수들은 복소수 벡터 :code:`v` 대해 허수부로 이루어진 벡터 view를 반환합니다. 

 :code:`gsl_vector_complex_const_imag()`  :code:`gsl_vector_complex_imag()`  동일한 기능을 제공합니다. 하지만 :code:`const` 정의된 벡터들에서만 사용가능합니다.
...
이 함수들은 배열에 대한 벡터 view를 반환합니다. 새 벡터의 값들은 :code:`n` 의 원소를 가지는 배열 :code:`base` 의해 결정됩니다. 수학적으로 새 벡터 :code:`v'`  :code:`i` 번째 원소는 다음과 같이 주어집니다.
...
.. code-block:: c
 ...
 :code:`i`  :math:`0` 부터 :code:`n-1`  범위를 가집니다.

 :code:`v` 원소들을 가지는 배열은 새로운 벡터 view에 속해져있지 않습니다. view가 소멸하더라도 배열은 여전히 존재합니다. 해당 배열이 속해있는 메모리는 해당 배열을 가르키는 :code:`base` 포인터가 소멸할 때 해제됩니다. 물론 반대로 배열이 소멸한 상황에서도 view는 여전히 사용할 수 있습니다.

함수 :code:`gsl_vector_const_view_array()`  :code:`gsl_vector_view_array()`  동일한 기능을 제공합니다. 하지만 :code:`const` 정의된 벡터들에서만 사용가능합니다.
...
이 함수는 보폭 값 :code:`stride` 이용해 배열 :code:`base` 서 생성된 벡터 view를 반환합니다. 이 부분 벡터는 :code:`gsl_vector_view_array()`  같은 방식으로 생성되고 :code:`n` 의 원소를 가지게 됩니다. 이는 원래 배열의 한 원소에서 다음 원소까지 :code:`stride` 간격을 가지도록 추출된 원소들입니다. 수학적으로 새 벡터 :code:`v'`  :code:`i` 째 원소는 다음과 같이 주어집니다.
...
유의할 점은 해당 벡터 view가 본래 배열에 대해 직접적인 접근 방법을 제공한다는 점입니다. 벡터 view들은 :code:`&view.vector`  사용해 벡터를 인자로 가지는 모든 함수들에 할당된 다른 벡터처럼 인자로 넣어줄 수 있습니다. 

함수 :code:`gsl_vector_const_view_array_with_stride()`  :code:`gsl_vector_view_array_with_stride()`  동일한 기능을 제공합니다. 하지만 :code:`const` 정의된 벡터들에서만 사용가능합니다.
...
이 함수는 벡터 :code:`src` 원소들을 :code:`dest` 벡터로 복사합니다. 이 두 벡터들은 같은 길이를 가져야합니다.

>int **gsl_vector_swap**(gsl_vector * *v*, gsl_vector * *w*)

이 함수들은 두 벡터 :code:`v` :code:`w`  원소들을 교환합니다. 이 두 벡터들은 같은 길이를 가져야합니다.
...
이 함수는 벡터 :code:`v` :code:`i` 째와 :code:`j` 째 원소들을 교환합니다.

>int **gsl_vector_reverse**(gsl_vector * *v*)

이 함수는 벡터 :code:`v` 원소 순서를 역으로 바꿉니다.
...
이 함수는 벡터 :code:`a` 벡터 :code:`b` 원소를 합합니다.  :math:`a_i \leftarrow a_i + b_i`  값이 :code:`a` 벡터에 저장되며 :code:`b` 변하지 않습니다. 이 두 벡터들은 같은 길이를 가져야합니다.

>int **gsl_vector_sub**(gsl_vector * *a*, const gsl_vector * *b*)

이 함수는 벡터 :code:`a` 벡터 :code:`b` 원소를 뺍니다.  :math:`a_i \leftarrow a_i - b_i`  값이 :code:`a` 벡터에 저장되며 :code:`b` 변하지 않습니다. 이 두 벡터들은 같은 길이를 가져야합니다.

>int **gsl_vector_mul**(gsl_vector * *a*, const gsl_vector * *b*)

이 함수는 벡터 :code:`a` 벡터 :code:`b` 원소를 곱합니다.  :math:`a_i \leftarrow a_i * b_i`  값이 :code:`a` 벡터에 저장되며 :code:`b` 변하지 않습니다. 이 두 벡터들은 같은 길이를 가져야합니다.

>int **gsl_vector_div**(gsl_vector * *a*, const gsl_vector * *b*)

이 함수는 벡터 :code:`a` 벡터 :code:`b` 원소를 나눕니다.  :math:`a_i \leftarrow a_i / b_i`  값이 :code:`a` 벡터에 저장되며 :code:`b` 변하지 않습니다. 이 두 벡터들은 같은 길이를 가져야합니다.

>int **gsl_vector_scale**(gsl_vector * *a*, const double *x*)

이 함수는 벡터 :code:`a` 원소들에 상수 계수 :code:`x` 곱합니다.  :math:`a_i \leftarrow x a_i`  값이 :code:`a` 저장됩니다.

>int **gsl_vector_add_constant**(gsl_vector * *a*, const double *x*)

이 함수는 벡터 :code:`a` 원소들에 상수 :code:`x` 더합니다.  :math:`a_i \leftarrow a_i + x`  값이 :code:`a` 저장됩니다.

>double **gsl_vector_sum**(const gsl_vector * *a*)

이 함수는 주어진 벡터 :code:`a` 원소들의 합  :math:`\sum_{i=1}^n a_i` 을 반환합니다.

>int **gsl_vector_axpby**(const double *alpha*, const gsl_vector * *x*, const double *beta*, gsl_vector * *y*)

이 함수는  :math:`y \leftarrow \alpha x + \beta y`  연산을 수행합니다. 벡터 :code:`x` :code:`y`  반드시 같은 길이를 가져야합니다.
...
이 함수는 벡터 :code:`v` 최댓값을 반환합니다.

>double **gsl_vector_min**(const gsl_vector * *v*)

이 함수는 벡터 :code:`v` 최솟값을 반환합니다.

>void **gsl_vector_minmax**(const gsl_vector * *v*, double * *min_out*, double * *max_out*)

이 함수는 벡터 :code:`v` 최댓값과 최솟값을 반환합니다. 해당 값들은 각각 :code:`min_out` :code:`max_out` 포인터가 가르키는 공간에 저장됩니다.

>size_t **gsl_vector_max_index**(const gsl_vector * *v*)

 이 함수는 벡터 :code:`v` 최댓값인 원소의 위치를 반환합니다. 동일한 최댓값이 여럿 존재하는 경우 가장 작은 값이 반환됩니다.

>size_t **gsl_vector_min_index**(const gsl_vector * *v*)

 이 함수는 벡터 :code:`v` 최솟값인 원소의 위치를 반환합니다. 동일한 최솟값이 여럿 존재하는 경우 가장 작은 값이 반환됩니다.

>void **gsl_vector_minmax_index**(const gsl_vector * *v*, size_t * *imin*, size_t * *imax*)

이 함수는 벡터 :code:`v` 최댓값과 최솟값의 위치들을 반환합니다. 해당 값들은 각각 :code:`imin` :code:`imax` 포인터가 가르키는 공간에 저장됩니다. 최댓값과 최솟값이 여럿 존재하는 경우 각각 가장 작은 값이 반환됩니다.

벡터 성질
--------------------------


다음 함수들은 실수, 복소수 벡터들에대해 모두 정의되어 있습니다. 복소수 벡터들은 실수, 허수부 모두 각각의 함수들의 조건들을 만족해야 합니다. 예를 들어서 복소수 벡터가 뒤의 나오는 함수 :code:`gsl_vector_ispos()` 서 참값이  :math:`1` 이 나왔다면 이는 해당 복소수 벡터의 실수부와 허수부 모두 양수라는 것을 의미합니다. 허수부나 실수부 둘 중 하나라도 음수 값이 있다면 해당 함수는  :math:`0` 을 반환할 것입니다. (*)
...
이 함수들은 벡터 :code:`v` 모든 원소들이  :math:`0` 이거나, 양수이거나, 음수이거나, 음수가 아닌 상황들을 판정해 참일 경우  :math:`1` 을 반환하고 아니면  :math:`0` 을 반환합니다.

>int **gsl_vector_equal**(const gsl_vector *u, const gsl_vector * *v*)

이 함수들은 주어진 두 벡터 :code:`u` :code:`v`  대해, 원소들을 비교해 이 둘이 같으면  :math:`1` 을 아니면  :math:`0` 을 반환합니다.

벡터 예제
--------------------------


다음 프로그램은 어떻게 벡터를 할당, 초기화하고 읽는 지를 보여줍니다. :code:`gsl_vector_alloc()`  :code:`gsl_vector_set()` 그리고 :code:`gsl_vector_get()`  사용합니다.
...
다음은 프로그램의 출력 결과입니다. 반복문의 마지막 단계에서 함수는 벡터 :code:`v` 길이 이상을 읽으려 시도하고 해당 시도는 :code:`gsl_vector_get()` 함수의 길이 확인 코드에 의해 감지되어 오류를 반환합니다.
...
프로그램을 실행하고 나면 :code:`test.dat` 파일이 생성됩니다. 이 파일은 :code:`v` 원소들을 값으로 가지고 있습니다. 해당 값들은 :code:`%.5g` 형식으로 저장되어져 있습니다. 해당 값은 :code:`gsl_vector_fscanf(f,v)`  사용해 다음과 같이 읽을 수 있습니다.
...
행렬은 :code:`gsl_matrix` 구조체로 정의됩니다. 이 구조체는 일반화된 블록들의 분할체 입니다. 벡터와 같이 이 구조체는 메모리의 특정 원소 집합을 나타냅니다. 그러나 벡터와는 달리 2개의 인덱스를 사용합니다.

>type **gsl_matrix**

 :code:`gsl_matrix` 구조체는 여섯개의 변수를 가지고 있습니다. 행렬의 두 차원, 물리적 차원, 행렬 원소들이 저장된 메모리를 가르키는 포인터 :code:`data` 행렬이 소유한 블럭 :code:`block` 그리고 소유권을 나타내는 :code:`owner` 니다. 물리적 차원은 메모리의 배치를 나타내며 행렬의 차원과는 다를 수 있습니다. 이는 부분 행렬의 사용을 위함합니다. :code:`gsl_matrix` 구조체는 다음과 같이 간단하게 정의되어 있습니다.
...
행렬은 행-우선 순서(row-major order)로 되어 있습니다. 이 의미는 한 행의 원소들은 메모리 내에 연속적으로 배치되어 있음을 의미합니다. 이 방법은 2차원 배열에 대한 표준 "C 언어 순서"입니다. 포트란의 경우 열-우선 순서(column-major order)로 되어 있습니다. 행의 갯수는 :code:`size1` 로 표기됩니다. 사용가능한 행 인덱스는  :math:`0` 에서 :code:`size1-1` 지 입니다. 같은 방식으로 열의 갯수는 :code:`size2` 표기됩니다. 열 인덱스는  :math:`0` 에서 :code:`size2-1` 지 입니다. 물리적인 행 차원은 :code:`tda` 표기됩니다. 이는 *trailing dimension*이라고도 불리며, 메모리 내 행렬의 배치에서 행의 크기를 나타냅니다.

예를 들어서 다음 행렬은 :code:`size1`  :math:`3` , :code:`size2`  :math:`4` , 그리고 :code:`tda`  :math:`8` 인 행렬입니다. 메모리 내의 행렬의 물리적 베치는 좌상단에서 부터 시작해 좌에서 우로 각 행들을 따라 값들이 배치됩니다.
...
메모리 내에서 사용되지 않는 영역은 " :code:`XX` 로 표기됩니다. 포인터 :code:`data` 메모리 내 행렬의 첫번째 원소의 위치를 가르킵니다. 포인터 :code:`block` 행렬의 원소들이 저장된 메모리 블럭의 위치를 나타냅니다. 만약 행렬이 해당 블럭을 가지고 있다면, :code:`owner` 는  :math:`1` 로 설정됩니다. 이 경우 블럭은 행렬이 해제되면 같이 해제됩니다. 만약, 행렬이 다른 객체가 소유한 블럭의 조각이라면 :code:`owner`  :math:`0` 으로 설정되고 해당 행렬이 가르키는 블럭은 해제되지 않습니다.

행렬을 할당하고 접근하는 함수들은 :code:`gsl_matrix.h`  정의되어 있습니다.
...
메모리에 행렬을 할당하는 함수들은 :code:`malloc` :code:`free` 형식을 따릅니다. 이 함수들은 그 자체로 오류 검증 기능을 가지고 있습니다. 행렬을 할당하기에 적절한 메모리가 없는 경우 GSL 오류 관리자를 호출해 :code:`GSL_ENOMEM` 오류 코드를 전달하고 :code:`Null` 포인터를 반환합니다. 따라서, 프로그램을 정지시키기기 위해 라이브러리의 오류 관리자를 사용한다면 모든 :code:`alloc` 검사할 필요가 없습니다.
...
이 함수는 행렬 :code:`a` :code:`b`  원소를 비교해서 같은 행렬이면  :math:`1` 을 아니면  :math:`0` 을 반환합니다.

>double **gsl_matrix_norm1**(const gsl_matrix * *a*)

이 함수는  :math:`m` - :math:`n`  행렬 :code:`A` 대해,  :math:`1` -노름값을 반환합니다. 이는 다음과 같이 정의됩니다.
...
아래의 프로그램은 함수 :code:`gsl_matrix_alloc()`  :code:`gsl_mtrix_set()` 그리고 :code:`gsl_matrix_get()`  이용해 행렬의 할당, 초기화, 그리고 값을 읽는 방법을 보여줍니다.
...
다음은 프로그램의 출력 결과입니다. 마지막 반복문은 행렬 :code:`m` 범위를 넘어 읽으려 시도하고, :code:`gsl_matrix_get()`  범위 확인 코드가 이를 감지했습니다.
...
프로그램의 실행이 끝나면, :code:`m` 행렬의 원소들을 담고 있는 파일 :code:`test.dat` 파일이 생성됩니다. 이 파일은 바이너리 형식으로 작성됩니다. 함수 :code:`gsl_matrix_fread()`  사용해 파일을 읽어 본래 행렬과 같은 행렬을 얻을 수 있습니다.
...
GSL의 블럭, 벡터 그리고 행렬 객체들은 C++의 :code:`valarraay` 모델을 따릅니다. 이 모델의 자세한 기술은 다음을 참조할 수 있습니다.
...
[range]: GNC C 컴파일러에서는 확장기능을 통해 범주 확인 기능을 지원합니다. 하지만, GCC의 기본 설치 목록에 포함되어 있지는 않습니다. 메모리 접근은 GCC의 :code:`gcc -fmudflap` 라는 메모리 보호 옵션과 Valgrind로 확인할 수 있습니다.
