...
:var:`1p`
   ...
   :math:`+1` 을 의미합니다. 
   예: 함수 :func:`log1p(x)` 나 :code:`k1p` 와 같은 변수. 
   이 변수는 :math:`=k+1` 을 의미합니다.

:var:`m1`
   
   :math:`-1` 을 의미합니다. 
   예: 함수 :func:`expm1(x)` 나 :code:`km1` 와 같은 변수. 
   이 변수는 :math:`=k-1` 을 의미합니다.
...
:var:`w` : 작업 공간을 가르키는 포인터.

:var:`state` : 상태 변수를 가르키는 포인터. ( 문자를 저장해야 한다면, :code:`s` 를 사용합시다.) 

:var:`result` : 결과(반환 값) 포인터.
...
:var:`size` : 배열이나, 백터의 크기. 예: :code:`double array[size]` 

:var:`stride` : 벡터의 stride

:var:`size1` : 행렬의 행 갯수.

:var:`size2` : 행렬의 열 갯수.

:var:`n` : 일반적인 정수. 예: 배열의 원소 숫자, fft 등등.

:var:`r` : 난수 발생자 (:code:`gsl_rng` ).

자료형 크기
-------------------------


ANSI C가 제공하는 :type:`int` 자료형은 16bit 크기를 보장함을 명심해야합니다 [#int_size]_ .
시스템에 따라 더 큰 크기를 제공할 수도 있지만 해당 자료형의 크기는 C에서 보장하지 않습니다. 
따라서, 32bit 크기의 자료형이 필요하다면 :type:`long int` 를 사용해야 합니다. 
이 데이터형은 최소 32bit의 크기를 보장합니다. 
물론 많은 플랫폼에서 :type:`int` 자료형의 크기가 32bit인 경우가 많습니다. 
...
모든 객체(예: 메모리 블럭)들은 :type:`size_t` 로 크기가 측정되어야 합니다. 
따라서, 모든 반복 과정(예: :code:`for(i=0; i<N; i++)` )은 :type:`size_t` 의 형태를 가지는 인덱스를 사용해야 합니다.

:type:`int` 와 :type:`size_t` 를 혼용하면 안됩니다. 이 둘은 교환 **불가능** 합니다.

감소하는 반복문을 사용하고 싶다면 주의해야 하는 데, :type:`size_t` 자료형은 부호가 없는 자료형이기 때문입니다. 
일반적인 감소 반복문보다는,
