...
:code:`GSL_ENOMEM` 오류 값를 넘기고 :code:`NULL` 포인터를 반환합니다. 
...
이를 위해서는 프로그램을 컴파일 할 때, 전처리기에서 매크로 :macro:`HAVE_INLINE` 처리 하도록 선언해야합니다.
...
.. function:: double gsl_vector_get (const gsl_vector * v, const size_t i)
  ...
  :macro:`HAVE_INLINE` 이 정의된 경우, 인라인 함수가 사용됩니다. 
...
행렬을 할당하기에 적절한 메모리가 없는 경우 GSL 오류 관리자를 호출해 :code:`GSL_ENOMEM` 오류 값를 전달하고 
...
.. function:: double gsl_matrix_get (const gsl_matrix * m, const size_t i, const size_t j)
   ...
   is returned. An inline version of this function is used when :macro:`HAVE_INLINE` is defined.
...
.. function:: void gsl_matrix_set (gsl_matrix * m, const size_t i, const size_t j, double x)
   ...
   handler is invoked. An inline version of this function is used when :macro:`HAVE_INLINE` is defined.
...
.. function:: double * gsl_matrix_ptr (gsl_matrix * m, size_t i, size_t j)
   ...
   and a null pointer is returned. Inline versions of these functions are used when :macro:`HAVE_INLINE` is defined.
