...
이 단원에서는 난수 분포를 생성하고 이들의 확률 분포를 계산하는 함수들에 대해 기술합니다.
이 단원에서 기술된 분포들의 특정 샘플은 라이브러리의 난수 발생기를 실행해 얻을 수 있습니다.
...
연속 난수 분포는 확률 밀도 함수로 정의됩니다. 확률 밀도 함수는 :math:`p(x)` 로 표기되며
:math:`x` 가 :math:`x` 에서 :math:`x + dx` 까지의 극소 범위에서 발생할 확률은 
:math:`p(x) dx` 로 표현할 수 있습니다.
...
누적 분포 함수 :math:`P(x)` 는 다음의 적분으로 정의되고

.. math:: P(x) = \int_{-\infty}^{x} dx' p(x')

확률 변수가 :math:`x` 보다 작을 확률을 나타냅니다.

보완(complementary) 누적 분포 함수 :math:`Q(x)` 는 다음 적분으로 정의되며 

.. math:: Q(x) = \int_{x}^{+\infty} dx' p(x')

확률 변수가 :math:`x` 보다 클 확률을 나타냅니다..

이 두 분포 함수는 :math:`P(x) + Q(x) = 1` 의 관계를 가지고 :math:`0 \le P(x) \le 1`,
:math:`0 \le Q(x) \le 1` 를 만족합니다.

역 누적 분포 :math:`x = P^{-1}(P)` 와 :math:`x = Q^{-1}(Q)` 는 
특정 :math:`P` 와 :math:`Q` 값에 대한 해당 :math:`x` 값을 나타냅니다.
이는 확률 변수들로부터 적절한 범위를 계산하는 데 사용될 수 있습니다. 

이상 확률 분포에서 정수값 :math:`k` 를 뽑아내는 확률은 :math:`p(k)` 로 주어지고 
:math:`p(k)`는 :math:`\sum_k p(k) = 1` 의 성질을 가집니다.
이산 누적 분포 :math:`P(k)` 는 다음과 같이 정의됩니다.

.. math:: P(k) = \sum_{i \le k} p(i) 

이 정의의 분포 값 급수는 :math:`k` 와 같거나 작을 확률을 나타냅니다.


똑같이 보완 누적 분포의 이산 변수 형태 :math:`Q(k)` 는 다음과 같이 정의되며

.. math:: Q(k) = \sum_{i > k} p(i) 

:math:`k` 보다 큰 샘플들의 확률 합을 나타냅니다. 이 두 정의는 :math:`P(k)+Q(k)=1` 관계를 만족합니다.
예를 들어 이산 분포의 범주가 :math:`1` 에서 :math:`n` 로 주어졌다면, :math:`P(n) = 1`, :math:`Q(n) = 0` 이고 :math:`P(1) = p(1)`,
:math:`Q(1) = 1 - p(1)` 가 됩니다.
...
.. function:: double gsl_ran_gaussian (const gsl_rng * r, double sigma)
   ...
   평균이 :math:`0` 이고 표준 편차가 :data:`sigma` 인 가우스 난수를 반환합니다.
   가우스 난수의 확률 분포는 가우스 분포라 불리며 :math:`x \in (-\infty, \infty)` 에 대해 다음과 같이 정의됩니다.

   .. math:: p(x) dx = {1 \over \sqrt{2 \pi \sigma^2}} \exp (-x^2 / 2\sigma^2) dx

   :math:`z = \mu + x` 변환을 이용해 평균이 :math:`\mu` 인 가우스 분포를 얻을 수도 있습니다.
   이 함수는 Box-Muller 알고리즘을 사용합니다. 이 알고리즘은 난수 생성자 :data:`r` 을 두 번에 걸쳐 
   호출합니다.

.. function:: double gsl_ran_gaussian_pdf (double x, double sigma)

   표준편차가 :data:`sigma` 가우스 분포에 대해 :data:`x` 의 확률 밀도 함수 :math:`p(x)` 를 계산합니다.
   위에 기술된 수식을 사용합니다.
...
.. function:: double gsl_ran_gaussian_ziggurat (const gsl_rng * r, double sigma)
   ...
   가우스 난수를 Marsaglia-Tsang 지구라트(ziggurat) 알고리즘과 Kinderman-Monahan-Leva 비율 방법을 사용해 계싼합니다.
   지구라트 알고리즘은 대부분의 경우 가장 빠른 속도를 보여줍니다.

.. function:: double gsl_ran_ugaussian (const gsl_rng * r)
              double gsl_ran_ugaussian_pdf (double x)
              double gsl_ran_ugaussian_ratio_method (const gsl_rng * r)

   단위-가우스 분포(unit Gaussian distribution)를 계산합니다. 이들은 위의 함수들이 :data:`sigma` = 1 인 표준 편차와 같습니다.
...
.. function:: double gsl_cdf_gaussian_P (double x, double sigma)
   ...
   표준 편차가 :data:`sigma` 인 가우스 분포에 대해 
   누적 분포 함수 :math:`P(x)`, :math:`Q(x)` 와 이들의 역함수를 계산합니다. 
