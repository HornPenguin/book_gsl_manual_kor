...
라이브러리에서는 radix-2 방법과 mixed-radix 방법을 제공합니다. 
효율성을 위해서 각 기능들은 복소수용 함수와 실수용 함수로 나뉘어져 있습니다. 
...
"GSL FFT Algorithms" 문서를 참고할 수 있습니다. (:ref:`참고 문헌과 추가 자료` 참고)
...
DFT는 연속 푸리에 변환을 근사할 필요가 있을 때 사용됩니다. 일반적으로 공간과 시간에 대해 이산 간격으로 샘플들이 주어졌을 때 사용합니다. 
...
완전히 같은 값을 얻을 수 있습니다(수치적 오류가 같이 나올 수 있습니다).

일반적으로, 변환-역변환 과정에서 지수 함수의 지수 부호를 :math:`\pm` 중 자유롭게 선택할 수 있습니다. 
:code:`GSL` 에서는 FFTPACK과 같은 규약을 따릅니다. 이 규약은 변환에서 음의 부호를 역변환에서는 양의 부호를 사용합니다. 
...
재구현체를 포함하고 있습니다. 이 방법에 관한 이론은 Clive Temperton 이 작성한
...
.. function:: gsl_fft_complex_wavetable * gsl_fft_complex_wavetable_alloc (size_t n)
   ...
   오류가 없다면 새로 할당된 :type:`gsl_fft_complex_wavetable` 의 포인터를 반환합니다.
...
.. function:: int gsl_fft_complex_forward (gsl_complex_packed_array data, size_t stride, size_t n, const gsl_fft_complex_wavetable * wavetable, gsl_fft_complex_workspace * work)
   ...
   =================================== =========================================================================================================
   :macro:`GSL_EDOM`                    자료의 길이 :data:`n` 이 양의 정수가 아닐 때, (예 :code:`n=0` ).
   :macro:`GSL_EINVAL`                  자료의 길이 :data:`n` 와 계산을 위해 사용하는 파동 참고표 :code:`wavetable` 가 맞지 않을 때.
   =================================== =========================================================================================================
...
때문에, 실수 푸리에 변환의 함수들은 두 개의 범주로 나뉘어집니다. 
...
.. [#GSLFFT] 이 문서는 :code:`GSL` 라이브러리에 포함되어 있습니다. :code:`doc/fftalgorithms.tex` 경로에 tex 문서로 되어있으며, 
              pdf는 `이곳 <https://github.com/Hyeansung/Hyeansung.github.io/raw/39627054317f26ae5915e2528112884848d9720e/resources/GSL_FFT_ALgorithm.pdf>`_ 에서 내려 받을 수 있습니다.
