...
기술합니다. 이 방법은 급수의 초기 항들에 외삽법을 적용해 수렴값을 근사해 
계산하고 근사 값의 오차도 같이 계산해 제공해 줍니다. :math:`u` 변환은 
수렴하는 급수, 발산하는 급수 그리고 점근적 확장을 표현하는 급수에도
사용할 수 있습니다.

이 단원의 함수들은 :file:`gsl_sum.h` 에 기술되어 있습니다.

가속 함수
======================

다음 함수들은 급수의 Levin :math:`u` 변환을 오차 값과 함께 계산합니다.
오차의 계산은 각 항부터 마지막 외삽법까지 계산과정에 있는 오차 전파를 이용해서 
추정합니다.
...
복잡도를 가지고 :math:`O(n)` 크기의 저장소만을 필요로 합니다.
