...
.. function:: gsl_eigen_herm_workspace * gsl_eigen_herm_alloc (const size_t n)
   ...
   찾기 위한 작업 공간을 메모리에 할당합니다.  The size of the workspace
   is :math:`O(3n)`.
...
켤례 복소수입니다.  이 과정에서 사용되는 알고리즘에는 이중 전이 프란시스 방법이 사용됩니다.
...
.. type:: gsl_eigen_nonsymmv_workspace
   ...
   nonsymmetric 고유 값 문제를 풀기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
...
.. function:: gsl_eigen_gen_workspace * gsl_eigen_gen_alloc (const size_t n)
   ...
   :code:`n` :math:`\times` :code:`n` 크기의 -- 행렬의 고유를
   찾기 위한 작업 공간을 메모리에 할당합니다.real generalized nonsymmetric eigensystems. The
