...
Quasi-상 삼각 행렬은 블럭 상 삼각 행렬로 대각 성분이 :math:`1 \times 1` 이나 :math:`2 \times 2` 크기의
블럭 행렬인 행렬을 의미합니다. 이 때, :math:`2 \times 2` 크기 행렬의 고유 값들은 :math:`A` 행렬의 고유 값들의
...
주어진 두 정사각 행렬 ( :math:`A`, :math:`B` ) 에 대해, 일반화된 비대칭 계의 풀이는 
다음을 만족하는 고유 값 :math:`\lambda` 와 고유 벡터 :math:`x` 를 찾는 문제입니다.


.. math:: A x = \lambda B x

고유 값 :math:`mu` 와 고유 벡터 :math:`y` 에 대해 다음과 같은
형식도 가능합니다.

.. math:: \mu A y = B y

이 두 문제는 :math:`\lambda = 1/\mu` 식을 이용하면 동일한 식임을 알 수 있습니다.
이때,  :math:`lambda` 와 :math:`mu` 는 모두 0이 아니여야 합니다. 
:math:`lambda` 가 0이여도 첫번째 식은 여전히 잘 정의된 고유계를 형성하지만
두번째 식은 :math:`mu` 가 정의되지 않습니다. 고유 값으로 0과 무한대를 사용하기 위해
라이브러리에서 제공하는 함수들은 다음과 같은 형태의 고유 계를 풀게 됩니다.


.. math:: \beta A x = \alpha B x

풀이 함수들은 :math:`\alpha` 와 :math:`\beta` 두 값을 반환해 사용자가
:math:`\lambda` 나 :math:`\mu` 를 계산해 사용할 수 있게 합니다.
각각은 나눗셈을 이용해서 :math:`\lambda = \alpha / \beta` 와
:math:`\mu = \beta / \alpha` 로 계산할 수 있습니다.


선형 행렬 pencil :math:`A - \lambda B` 의 행렬식이 모든 :math:`\lambda`
에 대해 0이면 해당 고유 계는 특이성을 가집니다. 
특이성을 가지는 고유 계는 :math:`\alpha = \beta = 0` 를 가지게 됩니다.
이는 해당 고유 계가 제대로 정의된 고유 값과 벡터를 가지지 않음을 의미합니다.
이 단원의 함수들은 특이성을 가지지 않는 일반적인 문제들에 대한 풀이를 제공합니다.
이러한 특이 pencil 값을 가지는 문제에 함수들을 적용할 경우 예기치 못한
결과가 나올 수 있습니다.


행렬 쌍 :math:`(A, B)` 의 실수 위의 일반화된 비 대칭 고유 계 문제는 
일반화된 슈어 분해와 관련되어 있습니다.
...
:math:`Q` 와 :math:`Z` 는 각각 좌, 우 슈어 벡터들의 직교 행렬입니다.
:math:`(S, T)` 는 일반화된 슈어 형태로 이들의 대각 성분은 각각 :math:`\alpha` 와 :math:`\beta`
값들을 제공해줍니다. 이 알고리즘들은 Moler & StewartQZ 의 QZ 방법에 기반해 있습니다. (참고 문헌을 확인할 수 있습니다.)
...
.. function:: gsl_eigen_gen_workspace * gsl_eigen_gen_alloc (const size_t n)
   ...
   :code:`n` :math:`\times` :code:`n` 크기의 실수 일반화된 비대칭 행렬의 고유 값을
   찾기 위한 작업 공간을 메모리에 할당합니다.
...
.. function:: void gsl_eigen_gen_params (const int compute_s, const int compute_t, const int balance, gsl_eigen_gen_workspace * w)
   ...
   :func:`gsl_eigen_gen` 을 사용하기 위한 계수들을 설정합니다.

   :data:`compute_s` 가 1로 정해지면 완전한 슈어 형태 :math:`S` 함수 :func:`gsl_eigen_gen` 에 의해
   계산됩니다. 0이 주어지면 :math:`S` 는 계산되지 않습니다. 0이 기본 설정입니다.
   :math:`S` 는 Schur 형태로 Quasi-상 삼각 행렬입니다.
   Quasi-상 삼각 행렬은 블럭 상 삼각 행렬로 대각 성분이 :math:`1 \times 1` 이나 :math:`2 \times 2` 크기의
   블럭 행렬인 행렬을 의미합니다. 이 때, :math:`2 \times 2` 크기 행렬의 고유 값들은 :math:`A` 행렬의 고유 값들의
   켤례 복소수입니다. :math:`1 \times 1` 행렬은 실수 고유 값과 대응됩니다.

   :data:`compute_t` 가 1로 정해지면 완전한 슈어 형태 :math:`T` 가 함수 :func:`gsl_eigen_gen` 에 의해
   계산됩니다. 0이 주어지면 :math:`T` 는 계산되지 않습니다. 0이 기본 설정입니다.
   :math:`T` 는 상 삼각 행렬로 대각선에 음수 값이
   없는 행렬입니다. :math:`S` 의 :math:`2 \times 2` 크기 블럭 행렬은 :math:`T` 의 
   :math:`2 \times 2` 크기 블럭 행렬에 대응됩니다.

   :data:`balance` 값은 현재 사용되지 않습니다. 잃반화된 균형 행렬기능은 아직 구현되지 않았습니다.

.. function:: int gsl_eigen_gen (gsl_matrix * A, gsl_matrix * B, gsl_vector_complex * alpha, gsl_vector * beta, gsl_eigen_gen_workspace * w)

   실수 위 일반화된 비대칭 행렬 쌍 ( :data:`A`, :data:`B` ) 의 고유 값을 계산해 
    ( :data:`alpha`, :data:`beta` )에 저장합니다. :data:`alpha` 는 복소수, 
    :data:`beta` 는 실수입니다. :math:`\beta_i` 가 0이 아니라면 
    :math:`\lambda = \alpha_i / \beta_i` 이 고유 값이 됩니다.
    비슷하게 :math:`\alpha_i` 가 0이 아니라면 :math:`\mu = \beta_i / \alpha_i` 이
    :math:`\mu A y = B y` 의 고유 값이 됩니다. :data:`beta` 의 원소들은 모두
    음수가 되지 않도록 정규화되어 있습니다.

    :math:`S` 가 필요하다면 이 행렬은 계산이 끝난 후 :data:`A` 에 저장되어 있습니다.
    :math:`T` 도 계산이 끝난 후 :data:`B` 에 저장됩니다. ( :data:`alpha`, :data:`beta` ) 
    의 고유 값 순서는 슈어 형태 :math:`S` 와 :math:`T` 의 대각선 블럭들 순서를 따릅니다.
    드문 일이지만 함수에서 모든 고유 값들을 찾지 못하는 경우가 있습니다. 이 경우 오류 값이 반환됩니다.
...
.. function:: int gsl_eigen_gen_QZ (gsl_matrix * A, gsl_matrix * B, gsl_vector_complex * alpha, gsl_vector * beta, gsl_matrix * Q, gsl_matrix * Z, gsl_eigen_gen_workspace * w)
   ...
   :func:`gsl_eigen_gen` 함수와 동일합니다. 다만 좌, 우 슈어 벡터들을 계산해
...
.. function:: gsl_eigen_genv_workspace * gsl_eigen_genv_alloc (const size_t n)
    ...
    실수 위 크기 :math:`n \times n` 의 일반화된 비 대칭 고유 계의 
    고유 값과 고유 벡터를 찾기 위한 작업 공간을 메모리에 할당합니다.
    공간의 크기는 :math:`O(7n)` 입니다.
...
.. function:: int gsl_eigen_genv (gsl_matrix * A, gsl_matrix * B, gsl_vector_complex * alpha, gsl_vector * beta, gsl_matrix_complex * evec, gsl_eigen_genv_workspace * w)
   ...
   실수 위 크기 :data:`n \times n` 의 일반화된 비 대칭 행렬 쌍( :data:`A` , :data:`B` )의 
   고유 값과 우 고유 벡터를 찾습니다. 고유 벡터들은 ( :data:`alpha` , :data:`beta` )에 
   저장되고 고유 벡터들은 :data:`evec` 에 저장됩니다. 이 함수는 먼저
   함수 :func:`gsl_eigen_gen` 를 호출해 고유 값들과 슈어 형태 그리고 슈어 벡터를 계산합니다.
   그 후 슈어 형태에 대한 고유 벡터를 찾고 이들을 슈어 벡터를 이용해 역변환 시켜
   원래 계의 고유 벡터들을 얻습니다. 슈어 벡터들은 계산과정에서 사용되 훼손됩니다.
   함수 :func:`gsl_eigen_genv_QZ` 를 사용하면 별도로 저장해둘 수 있습니다.
   계산된 고유 벡터는 크기 1을 가지도록 정규화 되어 있습니다. 
   계산 후 ( :data:`A` , :data:`B` ) 는 일반화된 슈어 형태 ( :math:`S` , :math:`T` )가 됩니다.
   함수 :func:`gsl_eigen_gen` 의 구동이 실패하면 고유 벡터들은 계산되지 않고
   오류 코드가 반환됩니다.
