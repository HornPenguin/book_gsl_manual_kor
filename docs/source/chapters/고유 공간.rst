...
복소수-일반화된 준 에르미트, 그리고 실수-일반화된 비대칭 고유계들의 풀이 기능들을 제공합니다.
...
.. type:: gsl_eigen_symm_workspace
   ...
   실수 대칭 행렬의 고유 값을 계산하기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
...
켤례 복소수입니다.  이 과정에서 사용되는 알고리즘에는 이중 전이 프란시스 방법(double-shiftted Francis method이 사용됩니다.
...
.. function:: void gsl_eigen_nonsymm_params (const int compute_t, const int balance, gsl_eigen_nonsymm_workspace * w)
   ...
   :func:`gsl_eigen_nonsymm` 함수를 이용해 풀기 위한 기본 계수들을 설정합니다.
   ...
   1.5--2 Flops의 시간을 필요로 합니다.

   :data:`balance` 가 1로 정해지면 고유 값을 계산하느 과정에서 규녛ㅇ 변환이
   행렬의 prior에 적용됩니다. 이 변환은 행렬의 행과 열을 
...
.. type:: gsl_eigen_nonsymmv_workspace
   ...
   비 대칭 행렬의 고유 값과 고유 벡터를 계산하기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
...
.. type:: gsl_eigen_gensymm_workspace
   ...
   generalized symmetric 고유 값 문제를 풀기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
   problems.
...
.. type:: gsl_eigen_gensymmv_workspace
   ...
   generalized symmetric 고유 값과 고유 벡터를 계산하기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
...
.. function:: gsl_eigen_gensymmv_workspace * gsl_eigen_gensymmv_alloc (const size_t n)
   ...
   eigensystems. 공간의 크기는 :math:`O(4n)` 입니다.
...
.. type:: gsl_eigen_genherm_workspace
   ...
   generalized hermitian 고유 값 문제를 풀기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
   problems.

.. function:: gsl_eigen_genherm_workspace * gsl_eigen_genherm_alloc (const size_t n)

   :code:`n` :math:`\times` :code:`n` 크기의 -- 행렬의 고유를
   찾기 위한 작업 공간을 메모리에 할당합니다.complex generalized hermitian-definite eigensystems. The
...
.. function:: gsl_eigen_genhermv_workspace * gsl_eigen_genhermv_alloc (const size_t n)
   ...
   eigensystems. 공간의 크기는 :math:`O(5n)` 입니다.
...
.. type:: gsl_eigen_genv_workspace
   ...
   generalized 고유값 문제를 풀기 위한 내부 인자들을 가지고 있는 작업 공간입니다.
   and eigenvector problems.
