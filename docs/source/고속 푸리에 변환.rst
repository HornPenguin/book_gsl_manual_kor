...
.. c:function:: int gsl_fft_halfcomplex_radix2_inverse (double data[], size_t stride, size_t n)
  ...
  radix-2 고속 푸리에 변환의 역변환과 조정 계수 없는 역변환을 주어진 :code:`stride` 간격과 길이 :code:`n` 가지는 반 복소 배열 :code:`data` 대해 계산합니다. 이때, :code:`data` :code:`gsl_fft_real_radix2()` 서 사용된 결과 배열과 동일한 저장 규칙을 가지고 있습니다. 결과로 나오는 실수 배열은 기본 배열 순서를 따릅니다.
