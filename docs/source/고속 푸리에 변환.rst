...
.. figure:: https://www.gnu.org/software/gsl/doc/html/_images/fft-complex-radix2.png
   :scale: 60%
...
.. function:: int gsl_fft_complex_forward (gsl_complex_packed_array data, size_t stride, size_t n, const gsl_fft_complex_wavetable * wavetable, gsl_fft_complex_workspace * work)
   ...
   :macro:`GSL_EINVAL`                 자료의 길이 :data:`n` 와 계산을 위해 사용하는 파동 참조표 :code:`wavetable` 가 맞지 않을 때.
   =================================== =========================================================================================================
...
이러한 대칭성을 가지는 수열을 **컬례 복소**(conjugage-complex)나 반 복소성(half-complex)을 가지고 있다고 부릅니다. 이러한 결과물의 구조적 차이점은 변환(실수->반 복소), 역변환(반 복소 -> 실수)에 대해 이전의 복소 푸리에 변환과 다른 설계를 필요로 합니다. 이에 따라, 실수 푸리에 변환의 함수들은 두 개의 범주로 나뉘어 집니다. 실수 배열을 처리하는 :code:`gsl_fft_real` 함수들과 반 복소 배열을 처리하는 :code:`gsl_fft_halfcomplex` 함수들입니다.
...
따라서 반 복소 배열의 계산에 필요한 것은 :code:`n` 크기의 실수 저장 공간뿐입니다. 계산 결과인 실수 배열도 입력 배열과 같은 크기의 배열에 저장됩니다.
...
complex[4].imag  = -halfcomplex[2]
 ...
 :code:`complex` 배열의 뒷 부분 , :code:`complex[3]` :code:`complex[4]` 대칭 조건으로 채워집니다. 대칭성에 의해 주파수 성분이 :code:`0` 부분의 허수부 :code:`complex[0].imag` :math:`0` 으로 정해집니다.
...
.. code-block:: bash
 ...
 :code:`complex` 배열의 뒷부분, :code:`complex[4]` :code:`complex[5]` 대칭 조건에 의해 결정됩니다. :code:`complex[0].imag` :code:`complex[3].imag` :math:`0` 으로 정해집니다.
